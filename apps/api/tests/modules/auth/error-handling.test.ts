...
describe('⚠️ Error Handling Tests', () => {
  ...
  describe('ERR-002: Validation Errors', () => {
    ...
    it('should handle weak password', async () => {
      ...
      expect(String(passwordError.message).toLowerCase()).toMatch(
        /8|uppercase|lowercase|number|symbol|stronger|password/,
      )
    })
  ...
  describe('ERR-003: Server Errors', () => {
    it('registers successfully when the database is reachable', async () => {
      const response = await request(await app())
        .post('/api/auth/register')
        .set(getAuthHeaders())
        .send({
          email: `db-ok-${Date.now()}@example.com`,
          username: `dbok${Date.now()}`,
          password: 'SecurePass123!',
        })
  ...
  describe('ERR-004: Network and Timeout Errors', () => {
    ...
    it('should handle concurrent requests', async () => {
      ...
      const promises = Array(5)
        .fill(null)
        .map(() =>
          request(expressApp).post('/api/auth/login').set(getAuthHeaders()).send({
            email,
            password: 'SecurePass123!',
          }),
        )
  ...
  describe('ERR-005: API Response Structure', () => {
    it('should maintain consistent error response structure', async () => {
      ...
      // requestId añadido vía res.json intercepción en addRequestMeta (no en ApiResponses static .send)
      if (response.body.meta && 'requestId' in response.body.meta) {
