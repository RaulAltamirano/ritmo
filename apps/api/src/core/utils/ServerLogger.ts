...
export class ServerLogger {
  ...
  private log(message: string, color?: string): void {
    const timestamp = this.config.showTimestamps
      ...
      : ''
    const colorCode = color && this.config.showColors ? this.colors[color] : ''
    const resetCode = color && this.config.showColors ? this.colors.reset : ''

    // Log message with timestamp and color
  }

  private logSection(section: LogSection): void {
