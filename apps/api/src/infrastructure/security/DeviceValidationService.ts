...
export class DeviceValidationService {
   ...
   */
  private validateECDSASignature(
    _signature: DeviceSignature,
    _expectedDeviceId: string,
    _sessionId: string,
  ): Promise<boolean> {
   ...
   */
  generateDeviceChallenge(sessionId: string): Promise<DeviceChallenge> {
  ...
  private isSuspiciousIP(_ipAddress: string): Promise<boolean> {
  ...
  private hasTimingAnomaly(_deviceInfo: DeviceInfo): boolean {
