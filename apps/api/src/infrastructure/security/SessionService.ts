...
export class SessionService {
  ...
  async detectSecurityThreats(
    ...
    try {
      ...
      if (threats.length > 0) {
        // Log security event
        await this.logSecurityEvent(sessionId, 'security_threat_detected' as any, {
          threats,
          riskLevel,
          currentRequest,
        })
    ...
    try {
      await this.prisma.securityLog.create({
        data: {
          ...
          severity: this.mapRiskLevelToSeverity(metadata.riskLevel || 'low'),
          metadata,
        },
      })
    } catch (error) {
  ...
  async preventSessionHijacking(
    ...
    try {
      ...
      if (!allowed) {
        ...
        // Log security event
        await this.logSecurityEvent(sessionId, 'session_hijacking_attempt', {
          reason,
          requestData,
          originalSession: {
