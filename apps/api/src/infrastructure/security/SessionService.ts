...
export class SessionService {
  ...
  private readonly _loggingService: StructuredLoggingService
   ...
   */
  private readonly consolidationStrategy: SessionConsolidationStrategy = {
  ...
  async consolidateDuplicateSessions(userId: string): Promise<void> {
    ...
    for (const [_deviceId, sessions] of sessionsByDevice) {
      if (sessions.length > 1) {
        ...
        const [_keepSession, ...duplicateSessions] = sessions
  ...
  async renewSessionIfNeeded(sessionId: string): Promise<{
    ...
    if (expiresIn < thirtyMinutes) {
      // Renew session
      const newExpiry = new Date(Date.now() + this.consolidationStrategy.sessionTimeout)
