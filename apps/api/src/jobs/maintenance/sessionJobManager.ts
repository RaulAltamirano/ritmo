...
export function initializeAllSessionJobs(): void {
  try {
    ...
    setInterval(runSessionCleanup, JOB_CONFIG.sessionCleanup.interval)

    // 2. Session Renewal - Medium priority (30 min)
    setInterval(runSessionRenewal, JOB_CONFIG.sessionRenewal.interval)

    // 3. Token Cleanup - Medium priority (1 hour)
    setInterval(runTokenCleanup, JOB_CONFIG.tokenCleanup.interval)

    // 4. Security Monitoring - Low priority (2 hours)
    setInterval(runSecurityMonitoring, JOB_CONFIG.securityMonitoring.interval)

    // 5. Session Analytics - Low priority (6 hours)
    setInterval(runSessionAnalytics, JOB_CONFIG.sessionAnalytics.interval)
...
export function stopAllSessionJobs(): void {
  // Note: setInterval returns numeric IDs, but we're not storing them
  // In a production environment, you'd want to store these IDs and clear them
}
