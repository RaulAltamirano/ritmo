...
async function cleanupOrphanedData(): Promise<number> {
  try {
    ...
    // 1. Clean up sessions without valid users
    const orphanedSessions = await prisma.userSession.deleteMany({
      where: {
        user: null, // Sessions without valid users
      },
    })
...
export function initializeTokenCleanup(): void {
  ...
  setInterval(runTokenCleanup, 60 * 60 * 1000) // 1 hour
}
