...
export class DeviceService {
  ...
  async getDeviceStats(userId: string): Promise<Record<string, unknown>> {
    const sessions = await prisma.userSession.findMany({
      where: { userId },
      select: { deviceId: true, sessionId: true, deviceType: true, isTrusted: true },
    })

    const deviceKeys = new Map<string, { trusted: boolean }>()
    for (const s of sessions) {
      const key = s.deviceId || s.sessionId
      const prev = deviceKeys.get(key)
      deviceKeys.set(key, { trusted: (prev?.trusted ?? false) || s.isTrusted })
    }
