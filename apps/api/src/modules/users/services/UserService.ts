...
import type { UserPreferences as UserPreferencesRow } from '@prisma/client'
import prisma from '../../../core/database/prisma.js'
import { ProfileUpdateDTO, UserDTO, UserPreferencesDTO } from '../dto/UserDTOs.js'
import { DEFAULT_TIMER_PRESETS, timerPresetsSchema } from '../timerPresets.dto.js'

interface UserWithPreferencesRow {
...
export class UserService {
  ...
  async getUserPreferences(userId: string): Promise<UserPreferencesDTO> {
    ...
    const row = user as typeof user & UserWithPreferencesRow
    ...
    if (row.preferences) {
      const storedPreferences = row.preferences as any
      ...
      if (
        storedPreferences.timerPresets !== null &&
        storedPreferences.timerPresets !== undefined
      ) {
        const parsed = timerPresetsSchema.safeParse(storedPreferences.timerPresets)
        if (parsed.success) {
          defaultPreferences.timerPresets = parsed.data
        }
      }
    }
  ...
  async updateUserPreferences(
    ...
    if (preferences.timerPresets !== undefined) {
      preferencesUpdateData.timerPresets = timerPresetsSchema.parse(
        preferences.timerPresets,
      )
    }
