  ...
  <div class="min-h-screen bg-canvas flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
      ...
      <div class="text-center">
        ...
        <!-- Redirect Message -->
        <BaseAlert v-if="$route.query.reason === 'authentication_required'" variant="info"
          title="Authentication Required" class="mt-4">
          You need to sign in to access that page
        </BaseAlert>

        <!-- Error Boundary -->
        <BaseAlert v-if="authError" variant="error" title="Authentication Error" class="mt-4">
          {{ authError }}
          <template #actions>
      ...
      <!-- Login Form -->
      <LoginForm :is-loading="isLoading" :general-error="error" @submit="handleLogin"
        @forgot-password="handleForgotPassword" @sign-up="handleSignUp" />
    </div>
  </div>
</template>
