<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header de página -->
    <PageHeader title="Analíticas" subtitle="Insights y progreso - Visualiza tu productividad" :actions="true">
      <template #actions>
        <BaseButton variant="outline" @click="exportData" class="flex items-center space-x-2">
          <Download :size="16" />
          <span>Exportar</span>
        </BaseButton>

        <BaseButton variant="primary" @click="showFilters = !showFilters" class="flex items-center space-x-2">
          <Filter :size="16" />
          <span>Filtros</span>
        </BaseButton>
      </template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <div class="bg-surface rounded-lg p-6 border border-outline">
      ...
      <div class="bg-surface rounded-lg p-6 border border-outline">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-bold text-gray-900 dark:text-white">
              {{ formatHours(stats.focusedHours) }}
            </p>
            <p class="text-sm text-green-600 dark:text-green-400">
              +8% vs semana pasada
            </p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
      ...
      <div class="bg-surface rounded-lg border border-outline p-6">
        ...
        <div class="h-64 flex items-end justify-between space-x-2">
          <div v-for="(day, index) in weeklyData" :key="index" class="flex-1 flex flex-col items-center">
            ...
            <div class="w-full bg-gradient-to-t from-primary-600 to-primary-400 rounded-t"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="bg-surface rounded-lg border border-outline p-6">
        ...
        <div class="space-y-3">
          <div v-for="hour in productiveHours" :key="hour.time"
            class="flex items-center justify-between p-3 bg-surface-raised rounded-lg">
            <div class="flex items-center space-x-3">
            ...
            <div class="flex items-center space-x-2">
              <div class="w-16 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                <div class="h-2 bg-green-500 rounded-full" :style="{ width: `${hour.productivity}%` }"></div>
              </div>
      ...
      <div class="bg-surface rounded-lg border border-outline p-6">
        ...
        <div class="space-y-4">
          <div v-for="achievement in recentAchievements" :key="achievement.id" class="flex items-start space-x-3">
            <div
              class="w-8 h-8 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
...
definePageMeta({
  title: 'Analytics',
  description: 'Insights and progress - Visualize your productivity patterns and achievements',
})
...
const recommendations = ref([
  {
    ...
    description:
      'Tu productividad es más alta entre 9-11 AM. Programa tareas importantes en ese horario.',
  },
  {
    id: 2,
    title: 'Reduce las distracciones',
    description:
      'Considera usar el modo enfoque más frecuentemente para mejorar tu concentración.',
  },
  {
    id: 3,
    title: 'Balance trabajo-estudio',
    description:
      'Tu tiempo de estudio ha disminuido. Considera dedicar más tiempo a esta categoría.',
  },
])
