  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
          <BaseButton variant="outline" size="sm" @click="$router.back()" aria-label="Volver">
        ...
        <div class="flex items-center space-x-3">
          <BaseButton variant="outline" size="sm" @click="editSchedule" aria-label="Editar horario">
            <Edit :size="16" class="mr-2" />
            Editar
          </BaseButton>

          <BaseButton variant="primary" size="sm" @click="showCreateTaskModal = true" aria-label="Crear tarea">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-surface rounded-xl border border-outline p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-1">
                <span v-for="day in schedule?.days" :key="day"
        ...
        <!-- Lista de tareas -->
        <div class="bg-surface rounded-xl border border-outline">
          <div class="p-6 border-b border-outline">
            <div class="flex items-center justify-between">
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
                Tareas relacionadas
              </h2>
              <BaseButton variant="outline" size="sm" @click="showCreateTaskModal = true" aria-label="Agregar tarea">
                <Plus :size="16" class="mr-2" />
                Agregar
              </BaseButton>
            </div>
          ...
          <div class="p-6">
            <div v-if="relatedTasks.length === 0" class="text-center py-8">
              <div
                class="w-16 h-16 bg-surface-overlay rounded-full flex items-center justify-center mx-auto mb-4">
              ...
              <BaseButton variant="primary" @click="showCreateTaskModal = true">
                Crear tarea
              </BaseButton>
            </div>

            <div v-else class="space-y-3">
              <TaskItem v-for="task in relatedTasks" :key="task.id" :task="task" @toggle-complete="handleToggleComplete"
                @start-timer="handleStartTimer" @stop-timer="handleStopTimer" @edit="handleEditTask"
                @delete="handleDeleteTask" @archive="handleArchiveTask" @add-note="handleAddNote"
                @update-task="handleUpdateTask" @start-with-mode="handleStartWithMode" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-1">
        <AIChatInterface :schedule-id="scheduleId" :initial-context="aiContext" @create-task="handleCreateTask"
          @add-note="handleAddNote" @update-schedule="handleUpdateSchedule" />
      </div>
    </div>
...
import { ArrowLeft, CheckSquare, Edit, Plus } from 'lucide-vue-next'
...
useHead({
  ...
  meta: [{ name: 'description', content: 'Gestiona tu horario con IA asistente' }],
})
...
const getCategoryColor = (category: string): string => {
  ...
  return (
    ...
    'bg-surface-overlay text-gray-800 dark:text-gray-200'
  )
}
