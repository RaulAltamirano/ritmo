<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Page Header -->
    <PageHeader title="Schedule" subtitle="Organize and manage your schedule efficiently" :actions="true">
      <template #actions>
        <BaseButton variant="primary" class="flex items-center space-x-2" @click="showQuickCreateModal = true">
          <Plus :size="16" />
          <span>New Event</span>
        </BaseButton>
      </template>
    ...
    <!-- Main Calendar View -->
    <CalendarViewComponent :events="calendarEvents" :schedules="schedules" :projects="projects"
      :initial-view="currentView" :show-filters="false" @event-click="handleEventClick" @date-click="handleDateClick"
      @event-create="handleCreateEvent" @event-update="handleUpdateEvent" @event-delete="handleDeleteEvent"
      @navigate-schedule="navigateToSchedule" />

    <!-- Quick Create Modal -->
    <QuickEventModal v-model="showQuickCreateModal" :selected-date="selectedDate" :selected-time="selectedTime"
      :available-projects="projects" @create-event="handleCreateEvent" />

    <!-- Event Detail Modal -->
    <EventDetailModal v-model="showEventDetailModal" :event="selectedEvent" :projects="projects" :tasks="tasks"
      @edit-event="handleEditEvent" @delete-event="handleDeleteEvent" @navigate-schedule="navigateToSchedule"
      @view-task="handleViewTask" />

    <!-- Edit Event Modal -->
    <QuickEventModal v-model="showEditModal" :selected-date="selectedDate" :selected-time="selectedTime"
      :available-projects="projects" @create-event="handleUpdateEvent" />
  </div>
</template>
  ...
  meta: [
    {
      name: 'description',
      content:
        'Organize and manage your schedule efficiently with the smart calendar.',
    },
  ],
})
...
const navigateToSchedule = (scheduleId: string) => {
  navigateTo(`/schedule/${scheduleId}`)
}
