<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
      <!-- Page Header -->
      <PageHeader title="Session Management" subtitle="Manage your active sessions across all devices and browsers"
        class="mb-4 sm:mb-6" :breadcrumbs="breadcrumbs">
        <template #actions>
          <BaseButton @click="goToProfile" variant="secondary" :icon="ArrowLeft" icon-position="left">
            Back to Profile
          </BaseButton>
        </template>
      </PageHeader>
      ...
      <BaseCard variant="minimalist" class="mb-4 sm:mb-6">
        <template #header>
          <div class="flex items-center space-x-3 sm:space-x-4">
            <div
              ...
              <ClientIcon name="monitor" class="w-5 h-5 sm:w-6 sm:h-6 text-green-600 dark:text-green-400" />
            </div>
            <div class="min-w-0 flex-1">
              <h3 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">Your Current Device</h3>
              <p class="text-sm sm:text-base text-gray-500 dark:text-gray-400 mt-1">Information about your current
                session</p>
            </div>
          </div>
        </template>
        ...
        <!-- Device Info Grid -->
        <div class="mt-4 sm:mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
          <div
            class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 sm:p-5 hover:bg-gray-100 dark:hover:bg-gray-800/70 transition-colors duration-200">
            <div class="flex items-center space-x-2 mb-3">
              <ClientIcon name="smartphone" class="w-4 h-4 text-gray-500 dark:text-gray-400" />
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Device</span>
            </div>
            <p class="text-sm sm:text-base font-medium text-gray-900 dark:text-white leading-relaxed">
          ...
          <div
            class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 sm:p-5 hover:bg-gray-100 dark:hover:bg-gray-800/70 transition-colors duration-200">
            <div class="flex items-center space-x-2 mb-3">
              <ClientIcon name="globe" class="w-4 h-4 text-gray-500 dark:text-gray-400" />
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Browser</span>
            </div>
            <p class="text-sm sm:text-base font-medium text-gray-900 dark:text-white leading-relaxed">
          ...
          <div
            ...
            <div class="flex items-center space-x-2 mb-3">
              ...
              <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">System</span>
            </div>
            <p class="text-sm sm:text-base font-medium text-gray-900 dark:text-white leading-relaxed">
          ...
          <div
            class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 sm:p-5 hover:bg-gray-100 dark:hover:bg-gray-800/70 transition-colors duration-200">
            <div class="flex items-center space-x-2 mb-3">
              <ClientIcon name="monitor" class="w-4 h-4 text-gray-500 dark:text-gray-400" />
              <span
                class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Resolution</span>
            </div>
            <p class="text-sm sm:text-base font-medium text-gray-900 dark:text-white leading-relaxed">
        ...
        <div class="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-700">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
            <div class="flex items-center space-x-2 sm:space-x-3">
              <ClientIcon name="hash" class="w-4 h-4 text-gray-400 dark:text-gray-500" />
              <span class="text-sm sm:text-base text-gray-600 dark:text-gray-300">Device ID:</span>
              <code
                class="bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded text-xs sm:text-sm font-mono text-gray-800 dark:text-gray-200">
          {{ currentDeviceFingerprint.deviceId.slice(0, 8) }}...
        </code>
            </div>
            <div class="flex items-center space-x-2 sm:space-x-3">
              <ClientIcon name="clock" class="w-4 h-4 text-gray-400 dark:text-gray-500" />
              <span class="text-sm sm:text-base text-gray-600 dark:text-gray-300">{{ currentDeviceFingerprint.timezone
              }}</span>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          <div class="flex items-start space-x-3">
            ...
            <p class="text-sm sm:text-base text-blue-700 dark:text-blue-300 leading-relaxed">
              <strong>Tip:</strong> This information helps you identify which of the sessions listed below is your
              current
              device.
            </p>
          </div>
      ...
      <BaseCard variant="minimalist">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center space-x-3 sm:space-x-4">
              <div
                ...
                <ClientIcon name="users" class="w-5 h-5 sm:w-6 sm:h-6 text-blue-600 dark:text-blue-400" />
              </div>
              <div class="min-w-0 flex-1">
                <h3 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">Active Sessions</h3>
                <p class="text-sm sm:text-base text-gray-500 dark:text-gray-400 mt-1">Manage your sessions across all
                  devices</p>
              </div>
            </div>
            <div class="flex-shrink-0">
              <BaseButton @click="refreshSessions" :loading="loadingSessions" loading-variant="dots" variant="secondary"
                :icon="RefreshCw" icon-position="left" size="sm">
        ...
        <div v-else-if="userSessions.length === 0" class="py-12 sm:py-16 text-center">
          <EmptyState title="No active sessions found"
            description="Your sessions will appear here when you log in from different devices" :icon="Monitor" />
        </div>

        <!-- Sessions Grid -->
        <div v-else class="mt-6 sm:mt-8 space-y-4 sm:space-y-6">
          <div v-for="session in userSessions" :key="session.id"
            class="group relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 sm:p-6 hover:shadow-md hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-200">
            ...
            <div class="flex items-start justify-between gap-4">
              ...
              <div class="flex items-start space-x-3 sm:space-x-4 flex-1 min-w-0">
                <div
                  ...
                  <ClientIcon name="monitor" class="w-6 h-6 sm:w-7 sm:h-7 text-gray-600 dark:text-gray-300" />
                </div>

                <div class="flex-1 min-w-0">
                  <div class="flex items-center space-x-2 sm:space-x-3 mb-2">
                    <h4 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white truncate">
                      {{ getDeviceName(session) }}
                    </h4>
                    <div class="w-2 h-2 sm:w-3 sm:h-3 rounded-full flex-shrink-0" :class="getStatusColor(session)">
                    </div>
                  </div>

                  <p class="text-sm sm:text-base text-gray-500 dark:text-gray-400 mb-3 sm:mb-4 leading-relaxed">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 text-xs sm:text-sm">
                    <div class="flex items-center space-x-1 sm:space-x-2">
                      <ClientIcon name="map-pin"
                        class="w-3 h-3 sm:w-4 sm:h-4 text-gray-400 dark:text-gray-500 flex-shrink-0" />
                      <span class="text-gray-600 dark:text-gray-300 truncate">{{ getLocationInfo(session) }}</span>
                    </div>
                    <div class="flex items-center space-x-1 sm:space-x-2">
                      <ClientIcon name="clock"
                        class="w-3 h-3 sm:w-4 sm:h-4 text-gray-400 dark:text-gray-500 flex-shrink-0" />
                      <span class="text-gray-600 dark:text-gray-300">{{ formatLastAccess(session) }}</span>
                    </div>
                    <div class="flex items-center space-x-1 sm:space-x-2">
                      <ClientIcon name="activity"
                        class="w-3 h-3 sm:w-4 sm:h-4 text-gray-400 dark:text-gray-500 flex-shrink-0" />
                      <BaseBadge :variant="getStatusBadgeVariant(session)" :content="getStatusText(session)"
                        size="xs" />
                    </div>
                  </div>
                </div>
              ...
              <div class="flex items-center flex-shrink-0">
                <BaseButton v-if="!isCurrentSession(session)" @click="openTerminateModal(session)" variant="error"
                  size="sm" :icon="X" icon-only title="Terminate session" />
        ...
        <template #footer>
          <div v-if="userSessions.length > 0"
            class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-6 sm:pt-8 border-t border-gray-200 dark:border-gray-700">
            <div class="flex items-center space-x-2 sm:space-x-3">
              <ClientIcon name="info" class="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 dark:text-gray-500" />
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
              <!-- Test Toast Button -->
              <BaseButton @click="testToast" variant="info" size="sm" :icon="Bell" icon-position="left">
                Test Toast
              </BaseButton>
              <BaseButton @click="openTerminateAllModal" variant="error" size="sm" :icon="LogOut" icon-position="left">
    ...
    <!-- Modals -->
    <TerminateSessionModal :is-open="showTerminateModal" :session="selectedSession" :loading="loadingTerminate"
      @update:is-open="showTerminateModal = $event" @close="closeTerminateModal" @confirm="terminateSession" />

    <TerminateAllSessionsModal :is-open="showTerminateAllModal"
      :session-count="userSessions.filter(s => !isCurrentSession(s)).length" :loading="loadingTerminate"
      @update:is-open="showTerminateAllModal = $event" @close="closeTerminateAllModal"
  ...
  try {
    ...
    if (result.success) {
      ...
      showToast({
        variant: 'error',
        title: 'Termination Failed',
        message: result.error || 'Failed to terminate other sessions. Please try again.',
        size: 'md',
        showIcon: true,
        autoDismiss: true,
        dismissDelay: 6000,
...
const testToast = () => {
  showToast({
    variant: 'success',
    title: 'Toast Test',
    message: 'This is a test toast notification to verify the system is working correctly.',
    size: 'md',
    showIcon: true,
    autoDismiss: true,
    dismissDelay: 4000,
