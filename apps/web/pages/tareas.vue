<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header de página -->
    <PageHeader title="Tareas" subtitle="Gestiona tus tareas y planes" :actions="true">
      <template #actions>
        <BaseButton variant="outline" @click="showFilters = !showFilters" class="flex items-center space-x-2">
          <Filter :size="16" />
          <span>Filtros</span>
        </BaseButton>

        <BaseButton variant="primary" @click="showTaskForm = true" class="flex items-center space-x-2">
          <Plus :size="16" />
      ...
      <div class="flex flex-col sm:flex-row gap-4">
        ...
        <div class="flex-1">
          <div class="relative">
            <input v-model="searchQuery" type="text" placeholder="Buscar tareas..."
              class="w-full pl-10 pr-4 py-2 border border-outline-strong rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-surface text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400" />
        ...
        <div class="flex gap-2">
          <select v-model="statusFilter"
            class="px-3 py-2 border border-outline-strong rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-surface text-gray-900 dark:text-white">
    ...
    <!-- Lista de tareas -->
    <div class="space-y-4">
      <div v-for="task in filteredTasks" :key="task.id" class="animate-fade-in">
        <TaskItem :task="task" @toggle-complete="toggleTaskComplete(task)" @start-timer="startTimer(task)"
          @stop-timer="stopTimer(task)" @edit="editTask(task)" @delete="deleteTask(task)">
          <!-- Acciones de tarea -->
          <div class="flex items-center space-x-2">
            <BaseButton variant="ghost" size="sm" @click="startTask(task)"
            ...
            <BaseButton variant="ghost" size="sm" @click="editTask(task)"
            ...
            <BaseButton variant="ghost" size="sm" @click="deleteTask(task.id)"
    ...
    <div v-if="showCreateModal || showEditModal"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" @click="closeModals">
      <div class="bg-surface rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" @click.stop>
        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">
          {{ showEditModal ? 'Editar Tarea' : 'Nueva Tarea' }}
        </h3>

        <TaskForm :initial-data="editingTask" :is-editing="showEditModal" @submit="handleTaskSubmit"
          @cancel="closeModals">
          <!-- Botones de acción -->
          <div class="flex items-center space-x-2">
            <BaseButton variant="outline" @click="closeTaskForm"> Cancelar </BaseButton>

            <BaseButton variant="primary" @click="saveTask" :disabled="!formData.name.trim()">
    ...
    <!-- Modo Focus -->
    <div v-if="isFocusMode && activeTask"
      ...
      <div class="text-center text-white p-8">
        ...
        <div class="flex items-center justify-center space-x-4">
          <BaseButton variant="outline" @click="pauseTimer"
            class="text-white border-white hover:bg-white hover:text-black">
            <Pause :size="20" />
            Pausar
          </BaseButton>

          <BaseButton variant="outline" @click="stopTimer"
            class="text-white border-white hover:bg-white hover:text-black">
            <Square :size="20" />
            Detener
          </BaseButton>
...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'Gestiona tus tareas y planes de manera eficiente',
    },
  ],
})

// El middleware se aplica globalmente en nuxt.config.ts
</script>
