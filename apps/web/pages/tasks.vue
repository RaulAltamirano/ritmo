<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header de página -->
    <PageHeader title="Tareas" subtitle="Gestiona tus tareas y planes" actions>
      <template #actions>
        <BaseButton
          variant="outline"
    ...
    >
      <div class="text-center text-white p-8 w-full max-w-lg">
        <h2 class="text-3xl font-bold mb-6">{{ activeTask.title }}</h2>

        <div class="w-full max-w-md mx-auto mb-8">
          <div
            class="h-3 rounded-full bg-white/20 overflow-hidden"
            role="progressbar"
            :aria-valuenow="focusProgressPercent"
            aria-valuemin="0"
            aria-valuemax="100"
            :aria-label="`${focusProgressPercent}% de la sesión completado`"
          >
            <div
          ...
          <p class="mt-3 text-sm text-white/70">
            {{ focusProgressPercent }}% completado
          </p>
        </div>
...
<script setup lang="ts">
  ...
  /** Duración total de la sesión (segundos) para la barra del modo Focus */
  const focusSessionTotalSec = computed(() => {
    const t = activeTask.value
    if (!t) return 1
    const parsed =
      typeof t.duration === 'string' ? Number.parseInt(t.duration, 10) : Number.NaN
    if (!Number.isNaN(parsed) && parsed > 0) return parsed * 60
    return Math.max(t.timeRemaining ?? 1, 1)
  })

  const focusProgressPercent = computed(() => {
    const t = activeTask.value
    const total = focusSessionTotalSec.value
    if (!t || total <= 0) return 0
    const elapsed = Math.max(0, total - (t.timeRemaining ?? 0))
    return Math.min(100, Math.round((100 * elapsed) / total))
  })
  ...
  const deleteTask = async (task: any) => {
    const confirmed = await confirm('Eliminar tarea', {
