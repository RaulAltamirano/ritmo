<template>
  ...
  >
    <div class="space-y-6">
      <div class="space-y-3 border-b border-gray-200 pb-5 dark:border-gray-800">
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="space-y-1">
            ...
            <h3 :id="stepHeadingId" class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ stepTitle }}
            </h3>
        ...
        <div class="h-1.5 overflow-hidden rounded-full bg-primary-100 dark:bg-primary-950/40">
          <div
      ...
      <section v-if="currentStep === 1" class="space-y-6" :aria-labelledby="stepHeadingId">
        <div class="space-y-3">
      ...
      <section v-else-if="currentStep === 2" class="space-y-6" :aria-labelledby="stepHeadingId">
        <div
        ...
        >
          ...
          <div class="grid grid-cols-5 gap-2">
            ...
            >
              <component
                :is="resolveOptionIcon(getRatingOption(metric.key, value as 1 | 2 | 3 | 4 | 5).iconKey)"
                class="h-4 w-4"
                aria-hidden="true"
              />
      ...
      <section v-else class="space-y-6" :aria-labelledby="stepHeadingId">
        ...
        <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-600 dark:border-gray-800 dark:bg-gray-900/60 dark:text-gray-300">
      ...
      <div class="flex w-full flex-col gap-3 sm:flex-row sm:justify-between">
        <BaseButton variant="secondary" size="sm" :disabled="loading" @click="handleRequestClose(false)">
          Ahora no
        </BaseButton>
        ...
        <div class="flex gap-3">
          ...
          <BaseButton
            ...
            :aria-label="currentStep === 1 ? 'Continuar al paso 2' : 'Continuar al paso 3'"
            @click="currentStep += 1"
          >
            Continuar
          </BaseButton>
...
const ratingMetrics = [
  { key: 'focusScore', label: '¿Qué tan concentrado estuviste?', ariaPrefix: 'Enfoque' },
  { key: 'progressScore', label: '¿Cuánto avanzaste de verdad?', ariaPrefix: 'Progreso' },
  { key: 'mentalDemand', label: '¿Qué tan demandante fue?', ariaPrefix: 'Carga mental' },
] as const
...
const selectCardClasses = (isSelected: boolean) =>
  [
    'flex min-h-[92px] flex-col items-center justify-center gap-2 rounded-2xl border px-4 py-4 text-center transition-all duration-200',
    isSelected
      ? 'border-gray-900 bg-gray-50 shadow-sm dark:border-gray-200 dark:bg-gray-800'
...
const ratingButtonClasses = (isSelected: boolean) =>
  [
    'flex h-14 flex-col items-center justify-center gap-1 rounded-xl border px-1 text-center text-sm transition-all duration-200',
    isSelected
      ? 'border-gray-900 bg-gray-900 text-white shadow-sm dark:border-gray-200 dark:bg-gray-100 dark:text-gray-900'
      : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300 dark:border-gray-800 dark:bg-gray-900/60 dark:text-gray-200 dark:hover:border-gray-700',
  ].join(' ')
...
const getRatingOption = (
  ...
  return (
    ratingOptions[metric].find(option => option.value === value) ?? ratingOptions[metric][2]
  )
}
