    ...
    <div class="space-y-4">
        ...
        <div class="flex items-center space-x-2 overflow-x-auto pb-2">
            ...
            <div class="flex space-x-2">
                <button v-for="filter in filters" :key="filter.type" @click="activeFilter = filter.type" :class="[
                ...
                <div class="p-4 border-b border-gray-100 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <div class="relative">
                                <div
                                    ...
                                    <span class="text-white font-semibold text-sm">{{ post.author.initials }}</span>
                                </div>
                            ...
                            <div>
                                <div class="flex items-center space-x-2">
                                    <h4 class="font-semibold text-gray-900 dark:text-white text-sm">{{ post.author.name
                                        }}</h4>
                                    <span class="text-xs text-gray-500 dark:text-gray-400">@{{ post.author.username
                                        }}</span>
                                </div>
                        ...
                        <div class="flex items-center space-x-1">
                            <button v-if="post.isPinned" class="p-1 text-yellow-500 hover:text-yellow-600"
                                title="Post fijado">
                                <Pin class="h-4 w-4" />
                            </button>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <button @click="toggleLike(post.id)" :class="[
                                ...
                                <Heart :class="post.interactions.isLiked ? 'fill-current' : ''" class="h-4 w-4" />
                            ...
                            <button @click="toggleComments(post.id)"
                                ...
                                <span class="text-sm font-medium">{{ post.interactions.comments }}</span>
                            </button>
                        ...
                        <!-- Guardar -->
                        <button @click="toggleBookmark(post.id)" :class="[
                            'p-2 rounded-lg transition-colors',
                            post.interactions.isBookmarked
                                ? 'text-blue-500 bg-blue-50 dark:bg-blue-900/20'
                                : 'text-gray-500 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20'
                        ]">
                            <Bookmark :class="post.interactions.isBookmarked ? 'fill-current' : ''" class="h-4 w-4" />
                        </button>
                    </div>
                </div>

                <!-- Comentarios expandidos -->
                <div v-if="expandedComments.includes(post.id)" class="border-t border-gray-100 dark:border-gray-700">
                    <div class="p-4 space-y-3">
                        <div v-for="comment in post.comments" :key="comment.id" class="flex items-start space-x-3">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center space-x-4 mt-2">
                                    ...
                                    <button class="text-xs text-gray-500 hover:text-blue-500 transition-colors">
                                        Responder
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center space-x-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <div class="flex-1">
                                <input v-model="newComment[post.id]" @keyup.enter="addComment(post.id)"
                                    placeholder="Escribe un comentario..."
                                    class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <CreatePostModal v-if="showCreatePost" @close="showCreatePost = false" @created="handlePostCreated" />
    </div>
</template>
