    ...
    >
      ...
      <div class="relative">
        ...
        <BaseButton
          ...
          variant="close"
          size="sm"
          class="absolute bottom-0 right-0 !p-2 rounded-full shadow-lg"
          :icon="Camera"
          icon-only
        ...
        <div v-if="avatarChanged" class="absolute bottom-0 right-0 flex space-x-2">
          ...
          <BaseButton
            @click="$emit('cancel-avatar-change')"
            variant="secondary"
            size="sm"
            class="!px-3 !py-2 text-xs"
          >
      ...
      <div class="text-center sm:text-left flex-1">
        ...
        <!-- Session Info -->
        <div class="mt-3 p-3 bg-surface-raised rounded-lg">
          <div
            class="flex items-center justify-center sm:justify-start space-x-4 text-sm"
          >
            ...
            <span class="text-gray-600 dark:text-gray-300"
              >Last access: {{ formatLastAccess() }}</span
            >
          </div>
        </div>
      </div>
