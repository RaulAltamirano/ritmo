        ...
        <div class="flex flex-col space-y-4">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              <div
                class="w-10 h-10 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center flex-shrink-0">
                <Lock class="w-5 h-5 text-gray-600 dark:text-gray-400" />
              </div>
              <h5 class="font-medium text-gray-900 dark:text-white text-sm sm:text-base">Password</h5>
            </div>
            <BaseBadge variant="success" content="Secure" left-icon="check" size="sm" />
          </div>

          <!-- Action -->
          <BaseButton @click="$emit('change-password')" variant="primary" size="sm" class="w-full">
            Change Password
          </BaseButton>
        </div>
      </BaseCard>
        ...
        <div class="flex flex-col space-y-4">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <h5 class="font-medium text-gray-900 dark:text-white text-sm sm:text-base">Two-Factor</h5>
            </div>
            <BaseBadge variant="warning" content="Disabled" left-icon="alert" size="sm" />
          </div>

          <!-- Action -->
          <BaseButton @click="$emit('enable-two-factor')" variant="primary" size="sm" class="w-full">
            Enable 2FA
          </BaseButton>
        </div>
      </BaseCard>
        ...
        <div class="flex flex-col space-y-4">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              ...
              <h5 class="font-medium text-gray-900 dark:text-white text-sm sm:text-base">Sessions</h5>
            </div>
            <ClientOnly>
              <BaseBadge v-if="sessionsLoaded && !loadingSessions" variant="info" :content="`${sessionCount} Active`"
                left-icon="info" size="sm" />
              <BaseBadge v-else-if="!sessionsLoaded" variant="info" content="View Sessions" left-icon="info"
                size="sm" />
              <span v-else class="animate-pulse bg-gray-300 dark:bg-gray-600 rounded w-16 h-5"></span>
            </ClientOnly>
          </div>

          <!-- Action -->
          <BaseButton @click="handleGoToSessions" variant="primary" size="sm" class="w-full">
            Manage Sessions
          </BaseButton>
        </div>
      </BaseCard>
