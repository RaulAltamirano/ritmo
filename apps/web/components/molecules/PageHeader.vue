<template>
  <div class="mb-8">
    ...
    <div
      v-if="phaseData"
      class="relative w-full h-48 sm:h-64 mb-6 rounded-xl overflow-hidden"
    >
      <img
        :src="phaseData.image"
        :alt="phaseData.label"
    ...
    <div
      ...
      :class="[actions ? 'sm:flex-row sm:items-center sm:justify-between' : '']"
    >
      <!-- Título y subtítulo -->
      <div class="flex-1 min-w-0">
        ...
        <p
          ...
          :class="[subtitleSizeClasses[size]]"
        >
          {{ subtitle }}
        </p>
      </div>
      ...
      <div
        ...
        :class="[actionsLayout === 'horizontal' ? 'flex-row' : 'flex-col sm:flex-row']"
      >
        <slot name="actions" />
      </div>
    </div>
...
<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<PageHeaderProps & { phaseKey?: string; darkMode?: boolean }>(),
    {
      size: 'lg',
      actions: false,
  ...
  const isGlobalDark = useDark({ storageKey: 'theme', initialValue: 'light' })
  ...
  const phaseData = computed(() =>
    props.phaseKey ? phaseMap[props.phaseKey] : undefined,
  )
  const darkMode = computed(() =>
    typeof props.darkMode === 'boolean' ? props.darkMode : isGlobalDark.value,
  )
</script>
