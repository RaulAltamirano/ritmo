<template>
  <div
    class="relative bg-surface/80 rounded-xl border transition-all duration-300 overflow-visible group backdrop-blur-sm"
    :class="[
      task.completed
        ...
        : task.isRunning
          ...
          : 'border-outline/50 hover:border-gray-200 dark:hover:border-gray-600/50 hover:shadow-lg',
    ]"
  >
    <div class="p-5">
      <div class="flex items-start space-x-4">
        ...
        <div class="flex items-center space-x-3">
          ...
          >
            <span
              class="text-lg font-mono font-bold text-purple-700 dark:text-purple-300"
            >
          ...
          <div class="flex items-center space-x-2">
            ...
            <button
              ...
              class="w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-200 shadow-sm border"
              :class="[
                task.isRunning
          ...
          <div class="relative menu-container" :data-task-id="task.id">
            <button
              ...
              class="menu-button w-10 h-10 rounded-lg bg-surface-raised/50 hover:bg-surface-overlay/50 text-gray-600 dark:text-gray-400 border border-outline-strong/30 flex items-center justify-center transition-all duration-200"
  ...
  <Teleport to="body">
    <div
      v-if="isMenuOpen"
      :style="menuPosition"
      class="fixed w-48 bg-surface rounded-xl shadow-2xl border border-outline py-2 z-[999999]"
      @click.stop
    >
      <button
        @click="handleEditClick"
        class="w-full px-4 py-3 text-left text-sm text-gray-700 dark:text-gray-300 hover:bg-surface-raised/50 flex items-center space-x-3 transition-all duration-200"
      >
        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path
    ...
    >
      ...
      <div class="p-4">
        <textarea
          ...
          class="w-full h-32 px-3 py-2 border border-outline-strong rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-colors bg-surface text-gray-900 dark:text-white resize-none"
        ></textarea>
      </div>
...
<script setup lang="ts">
  ...
  const getCategoryColor = (category: string): string => {
    const colors = {
      Trabajo:
        'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/30',
      Estudio:
        'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800/30',
      Personal:
        'bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800/30',
      Urgente:
  ...
  const toggleMenu = () => {
    ...
    window.dispatchEvent(
      new CustomEvent('close-other-menus', { detail: { exceptId: props.task.id } }),
    )
