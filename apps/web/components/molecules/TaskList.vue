<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      ...
      <TaskFilters :categories="availableCategories" :selected-category="selectedCategory"
        @filter-change="setCategoryFilter" />

      <!-- Vista y contador simplificados -->
      <ViewModeToggle v-model="viewMode" :task-count="filteredTasks.length" @view-change="viewMode = $event" />
    </div>

    <!-- Vista de lista usando TaskCard -->
    <div v-if="viewMode === 'list'" class="space-y-3 sm:space-y-4" role="list" aria-label="Lista de tareas disponibles">
      <TaskCard v-for="task in filteredTasks" :key="task.id" :task="convertToTaskItemFormat(task)"
        @start-timer="handleStartTimer(task)" @toggle-complete="handleToggleComplete(task)" />
    </div>

    <!-- Vista de cuadrícula usando TaskCard -->
    <div v-else class="grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3" role="list"
      aria-label="Cuadrícula de tareas disponibles">
      <TaskCard v-for="task in filteredTasks" :key="task.id" :task="convertToTaskItemFormat(task)"
        @start-timer="handleStartTimer(task)" @toggle-complete="handleToggleComplete(task)" />
    </div>

    <!-- Modal de notas -->
    <TaskNoteModal v-model="showNoteModal" :task-title="editingTask?.name || ''" :initial-note="editingTask?.notes"
      @save="handleSaveNote" />
    ...
    <EmptyState v-if="filteredTasks.length === 0" :selected-category="selectedCategory" />
  </div>
</template>
