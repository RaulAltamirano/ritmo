<template>
  <form @submit.prevent="handleSubmit" class="space-y-4">
    ...
    <div>
      ...
      />
      <p v-if="errors.title" id="title-error" class="mt-1 text-sm text-red-600">
        {{ errors.title }}
      </p>
    </div>
    ...
    <div class="flex space-x-3 pt-4">
      <Button type="button" variant="secondary" @click="$emit('cancel')" class="flex-1">
        Cancelar
      </Button>
      <Button type="submit" variant="primary" :disabled="isSubmitting" class="flex-1">
        <Icon v-if="isSubmitting" name="timer" size="sm" class="mr-2 animate-spin" />
