  ...
  <BaseCard variant="elevated" padding="lg">
    <div class="space-y-8">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div
          class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 p-4 rounded-xl"
        >
        ...
        <div
          class="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 p-4 rounded-xl"
        >
          <div class="flex items-center space-x-2 mb-2">
      ...
      <div class="space-y-4">
        ...
        <div
          ...
          role="group"
          aria-label="Gráfico de calor de actividad diaria"
        >
          <div v-for="(day, index) in dailyData" :key="index" class="text-center group">
            <div
              class="w-10 h-10 mx-auto rounded-lg flex items-center justify-center text-xs font-bold transition-all duration-300 cursor-pointer relative"
              :class="[getHeatmapColor(day.sessions)]"
            ...
            >
              ...
              >
                ...
                <div
                  class="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-gray-900 dark:border-t-gray-800"
                ></div>
              </div>
            </div>
...
<script setup lang="ts">
  ...
  import { Calendar, Target, TrendingUp } from 'lucide-vue-next'
  ...
  const getHeatmapColor = (sessions: number) => {
    if (sessions === 0)
      return 'bg-surface-overlay text-gray-400 dark:text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-600'
    if (sessions <= 2)
      return 'bg-green-200 dark:bg-green-800 text-green-700 dark:text-green-300 hover:bg-green-300 dark:hover:bg-green-700'
    if (sessions <= 4)
      return 'bg-green-400 dark:bg-green-700 text-white hover:bg-green-500 dark:hover:bg-green-600'
    if (sessions <= 6)
      return 'bg-green-600 dark:bg-green-600 text-white hover:bg-green-700 dark:hover:bg-green-500'
  ...
  const getWeekColor = (sessions: number) => {
    if (sessions === 0) return 'bg-surface-overlay text-gray-400 dark:text-gray-500'
    if (sessions <= 5)
