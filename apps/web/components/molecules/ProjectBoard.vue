<template>
  <div class="w-full">
    ...
    <div
      v-if="projectInfo"
      class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4 rounded-lg bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800/50"
    >
      <div class="flex-1">
        ...
        <div class="text-sm text-gray-700 dark:text-gray-200">
          Next review:
          <span class="font-medium">{{ formatDate(projectInfo.nextReview) }}</span>
        </div>
        <div class="text-sm text-gray-700 dark:text-gray-200 mt-1">
          Methodology:
          <span class="font-medium">{{ projectInfo.methodology ?? 'Kanban' }}</span>
        </div>
        <div class="text-sm text-gray-700 dark:text-gray-200 mt-1">
          Owner:
          <span class="font-medium">{{ projectInfo.owner ?? 'Unassigned' }}</span>
        </div>
        <div class="text-sm text-gray-700 dark:text-gray-200 mt-1">
          Created:
          <span class="font-medium">{{ formatDate(projectInfo.createdAt) }}</span>
        </div>
      </div>
      <div class="flex flex-col items-end gap-2">
        <div class="flex flex-wrap gap-2 items-center">
          <span
            ...
            class="px-3 py-1 rounded-full text-xs font-medium bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200"
            >Status: {{ projectInfo.status }}</span
          >
          <span
            v-if="projectInfo.progress !== undefined"
            class="px-3 py-1 rounded-full text-xs font-medium bg-green-200 dark:bg-green-700 text-green-800 dark:text-green-200"
            >Progress: {{ projectInfo.progress }}%</span
          >
        </div>
        ...
        <div class="flex items-center gap-2 mt-2">
          ...
          <template v-if="projectInfo.members && projectInfo.members.length">
            <div
              v-for="member in projectInfo.members"
              :key="member.id"
            ...
            >
              <img
                :src="member.avatar"
                :alt="member.name"
                class="w-7 h-7 rounded-full border-2 border-white dark:border-gray-800 shadow"
              />
    ...
    >
      ...
      <div class="flex flex-1 flex-wrap gap-2 justify-end items-center">
        ...
        <select
          ...
          class="px-3 py-2 rounded-lg border border-outline-strong bg-surface text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
        >
    ...
    <div
      class="grid gap-2 md:gap-4 py-4 min-h-[60vh] w-full"
      :class="[`grid-cols-1 md:grid-cols-2 lg:grid-cols-${columns.length}`]"
    >
      <div
        v-for="column in columns"
        :key="column.status"
        class="bg-surface-raised/90 rounded-lg p-2 md:p-4 flex flex-col flex-1 min-h-[40vh] w-full"
        :aria-label="column.label"
        role="region"
      >
        <div class="flex items-center justify-between mb-4">
          <h3 class="font-bold text-lg text-gray-700 dark:text-gray-100">
            {{ column.label }}
          </h3>
          <span class="text-xs text-gray-500 dark:text-gray-300"
            >{{ filteredTasksByStatus[column.status].length }} tasks</span
          >
        </div>
...
<script setup lang="ts">
  ...
  const filteredTasks = computed(() => {
    ...
    if (selectedUser.value) {
      tasks = tasks.filter(
        t => t.members && t.members.some((m: any) => m.id === selectedUser.value),
      )
    }
  ...
  const columnProgress = computed(() => {
    ...
    for (const col of columns.value) {
      result[col.status] = Math.round(
        (filteredTasksByStatus.value[col.status].length / total) * 100,
      )
    }
    return result
  })
