<template>
  <div class="calendar-filters border-t border-outline pt-4 mt-4">
        ...
        <div class="filter-group">
          ...
          <div class="flex items-center gap-2">
            ...
            >
              ...
              />
              <span class="priority-badge" :class="[priority.color]">
                {{ priority.label }}
              </span>
            </label>
          </div>
    ...
    <div
      ...
      class="advanced-filters mt-4 pt-4 border-t border-outline"
    >
...
<script setup lang="ts">
  ...
  const hasActiveFilters = computed(() => {
    return (
      localFilters.value.projects.length > 0 ||
  ...
  const activeFilterLabels = computed(() => {
    ...
    localFilters.value.projects.forEach((project: string) => {
      labels.push({ key: 'projects', value: project, label: `Proyecto: ${project}` })
    })

    localFilters.value.categories.forEach((category: string) => {
      labels.push({
        key: 'categories',
        value: category,
    ...
    localFilters.value.priorities.forEach((priority: string) => {
      const priorityLabel =
        priorities.find((p: { value: string; label: string }) => p.value === priority)
          ?.label || priority
      labels.push({
        key: 'priorities',
        value: priority,
    ...
    if (dateRange.value.start) {
      labels.push({
        ...
        label: `Desde: ${dateRange.value.start}`,
      })
    }
    ...
    if (dateRange.value.end) {
      labels.push({
        ...
        label: `Hasta: ${dateRange.value.end}`,
      })
    }
  ...
  const removeFilter = (key: string, value: string) => {
    switch (key) {
      case 'projects':
        localFilters.value.projects = localFilters.value.projects.filter(
          (p: string) => p !== value,
        )
        break
      case 'categories':
...
<style scoped>
  ...
  .filter-select {
    @apply px-3 py-2 border border-outline-strong rounded-lg bg-surface text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors;
  }
