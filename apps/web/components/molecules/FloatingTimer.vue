    ...
    >
      ...
      >
        ...
        >
          ...
          <!-- Contenido del panel -->
          <div id="timer-details-content" class="space-y-4">
          ...
          <!-- Acciones -->
          <div class="mt-4 pt-3 border-t flex gap-2" :class="detailsBorderClasses">
            <button
...
<script setup lang="ts">
  ...
  const timerIconClasses = computed(() => ({
    ...
    'text-orange-500 dark:text-orange-400':
      timeLeft.value > 300 && timeLeft.value <= 600,
  ...
  const detailsFocusRingClasses = computed(
    () => 'focus:ring-offset-white dark:focus:ring-offset-gray-800',
  )
  ...
  const getProgressText = (): string => {
    ...
    const progress = Math.round(
      ((totalTime.value - timeLeft.value) / totalTime.value) * 100,
    )
  ...
  const getTaskType = (): string => {
    ...
    if (
      taskName.includes('estudiar') ||
      taskName.includes('leer') ||
      taskName.includes('repasar')
    ) {
