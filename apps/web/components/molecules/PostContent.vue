    ...
    <div class="space-y-4">
        ...
        <div v-if="post.content.type === 'technique'" class="space-y-4">
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                <div class="flex items-center space-x-2 mb-2">
                    ...
                    <span class="text-sm font-medium text-blue-700 dark:text-blue-300">Experiencia Personal</span>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                    <div class="flex items-center space-x-2 mb-1">
                        ...
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Efectividad</span>
                    </div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ post.content.effectiveness }}</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                    <div class="flex items-center space-x-2 mb-1">
                        <Clock class="h-4 w-4 text-gray-600 dark:text-gray-400" />
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Tiempo Requerido</span>
                    </div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ post.content.timeRequired }}</p>
                </div>
            </div>
            ...
            <div v-if="post.content.sources.length > 0">
                <div class="flex items-center space-x-2 mb-3">
                    ...
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Fuentes y Recursos</span>
                </div>
                <div class="space-y-2">
                    <div v-for="source in post.content.sources" :key="source.title"
        ...
        <div v-else-if="post.content.type === 'music'" class="space-y-4">
            <div class="flex items-start space-x-4">
                ...
                <div class="flex-1">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ post.content.title }}</h3>
                    <p class="text-gray-600 dark:text-gray-400">{{ post.content.artist }}</p>
                    <p v-if="post.content.album" class="text-sm text-gray-500 dark:text-gray-500">{{ post.content.album
                        }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
                    ...
                    <div class="text-xs text-gray-600 dark:text-gray-400">{{ post.content.genre }}</div>
                </div>
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
                    <div class="text-sm font-medium text-gray-700 dark:text-gray-300">Fase</div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">{{ post.content.studyPhase }}</div>
                </div>
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
                    <div class="text-sm font-medium text-gray-700 dark:text-gray-300">Duración</div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">{{ post.content.duration }}</div>
                </div>
                <div v-if="post.content.bpm" class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-center">
                    <div class="text-sm font-medium text-gray-700 dark:text-gray-300">BPM</div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">{{ post.content.bpm }}</div>
                </div>
            </div>

            <!-- Mood tags -->
            <div v-if="post.content.mood.length > 0" class="flex flex-wrap gap-2">
                <span v-for="mood in post.content.mood" :key="mood"
            ...
            <div class="flex space-x-3">
                <a v-if="post.content.spotifyUrl" :href="post.content.spotifyUrl" target="_blank"
                ...
                <a v-if="post.content.youtubeUrl" :href="post.content.youtubeUrl" target="_blank"
                    rel="noopener noreferrer"
        ...
        <div v-else-if="post.content.type === 'image'" class="space-y-4">
            ...
            <div class="relative">
                <img :src="post.content.imageUrl" :alt="post.content.title"
                    class="w-full h-64 object-cover rounded-lg" />
                <div class="absolute top-3 left-3">
                    <span class="px-2 py-1 bg-black/50 text-white rounded-full text-xs font-medium">
                        {{ post.content.category }}
                    </span>
                </div>
            </div>
            ...
            <div v-if="post.content.tags.length > 0" class="flex flex-wrap gap-2">
                <span v-for="tag in post.content.tags" :key="tag"
                    class="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-full text-xs">
                    #{{ tag }}
                </span>
            </div>
        ...
        <div v-else-if="post.content.type === 'trophy'" class="space-y-4">
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-2">
                    <h4 class="font-semibold text-gray-900 dark:text-white">{{ post.content.achievement.name }}</h4>
                    <span
                ...
                <p class="text-sm text-gray-700 dark:text-gray-300 mb-3">{{ post.content.achievement.description }}</p>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ post.content.achievement.points }}
                        puntos</span>
            ...
            <div v-if="post.content.studySession" class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                <h5 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Sesión de Estudio</h5>
                <div class="grid grid-cols-3 gap-3 text-center">
                    <div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Duración</div>
                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{
                            post.content.studySession.duration }}</div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Técnica</div>
                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{
                            post.content.studySession.technique }}</div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Materia</div>
                        <div class="text-sm font-medium text-gray-900 dark:text-white">{{
                            post.content.studySession.subject }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-else-if="post.content.type === 'review'" class="space-y-4">
                ...
                <div class="flex items-center space-x-3 mb-3">
                    <div class="flex items-center">
                        <Star v-for="i in 5" :key="i"
                            :class="i <= post.content.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'"
                            class="h-4 w-4" />
                    </div>
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ post.content.rating }}/5</span>
                    <span
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-3">
                    <h5 class="text-sm font-medium text-green-700 dark:text-green-300 mb-2">✅ Pros</h5>
                ...
                <div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-3">
                    <h5 class="text-sm font-medium text-red-700 dark:text-red-300 mb-2">❌ Contras</h5>
                    <ul class="text-sm text-gray-700 dark:text-gray-300 space-y-1">
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
                <h5 class="text-sm font-medium text-blue-700 dark:text-blue-300 mb-2">💡 Recomendación</h5>
                <p class="text-sm text-gray-700 dark:text-gray-300">{{ post.content.recommendation }}</p>
        ...
        <div v-else-if="post.content.type === 'article'" class="space-y-4">
            ...
            <div v-if="post.content.featuredImage" class="relative">
                <img :src="post.content.featuredImage" :alt="post.content.title"
            ...
            <div v-if="post.content.externalUrl" class="flex justify-center">
                <a :href="post.content.externalUrl" target="_blank" rel="noopener noreferrer"
