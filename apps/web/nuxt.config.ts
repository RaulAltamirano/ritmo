      ...
      script: [
        {
          innerHTML: `
            (function() {
              ...
              // color-scheme tells the browser to render native UI controls
              // (scrollbars, inputs, selects) in the correct mode immediately.
              document.documentElement.style.colorScheme = isDark ? 'dark' : 'light';

              // Update theme-color meta so the browser chrome (mobile address
              // bar, PWA title bar) matches the page background.
              var themeColorMeta = document.querySelector('meta[name="theme-color"]');
              if (themeColorMeta) {
                themeColorMeta.setAttribute('content', isDark ? '#0f172a' : '#14b8a6');
              }
