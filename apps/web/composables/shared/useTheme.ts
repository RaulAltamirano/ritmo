...
//
// En la app Nuxt, preferir `$theme` de `plugins/theme.client.ts`. Llamar
// `useTheme()` directamente en la UI duplica estado y watchers.
...
export interface ThemeOptions {
  ...
  /**
   * Sincronización con `/users/preferences`. En la app Nuxt el plugin
   * `theme.client.ts` lo deja en `false` salvo que producto active sync.
   * @default false — alineado con el tema global; pon `true` en demos/tests que lo requieran.
   */
  enableApiSync?: boolean
  /**
   * @default false — mismo criterio que `enableApiSync`
   */
...
export function useTheme(options: ThemeOptions = {}) {
  ...
  const toggleTheme = () => {
    if (hasUserPreference.value) {
      ...
      void setTheme(isDark.value ? 'light' : 'dark')
    } else {
  ...
  if (typeof document !== 'undefined') {
    const applyThemeChrome = (newIsDark: boolean) => {
