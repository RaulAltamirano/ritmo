...
describe('useSortableKeyboard', () => {
  it('grabbing an item sets it as grabbed', () => {
    ...
    const { onGripKeydown, isGrabbed } = useSortableKeyboard({ items, onReorder: vi.fn() })
  ...
  it('ArrowDown while grabbed moves item one position down and updates announce', () => {
    const items = ref([makeTask('a', 'Task A'), makeTask('b', 'Task B'), makeTask('c', 'Task C')])
    const onReorder = vi.fn()
    const { onGripKeydown, isGrabbed, announce } = useSortableKeyboard({ items, onReorder })
  ...
  it('announce says no positions available when list has one item', () => {
    ...
    const { onGripKeydown, announce } = useSortableKeyboard({ items, onReorder: vi.fn() })
