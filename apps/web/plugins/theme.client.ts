...
export default defineNuxtPlugin(() => {
    ...
    return {
      provide: {
        theme: {
          ...
          setTheme: (_: 'light' | 'dark' | 'system') => Promise.resolve(),
          toggleTheme: () => {},
          resetToSystem: () => {},
  ...
  // `enableApiSync` / `autoSyncOnChange`: misma postura que los defaults de
  // `useTheme` (false) hasta que producto active persistencia de tema en API.
