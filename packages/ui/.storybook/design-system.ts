 ...
 *
 * Clases de componentes (p. ej. tarjetas de opción en rejilla) se definen en
 * `src/tokens/colors.ts` — `semanticColors.interactiveOption` y
 * `getInteractiveOptionClasses` (export en `src/tokens/colors.ts` / `tokens/index.ts`)
 * — reutilizadas en la app (check-in, feedback).
 */
