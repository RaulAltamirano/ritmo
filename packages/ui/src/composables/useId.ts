...
export function useId(prefix = 'id', props?: Record<string, any>) {
...
export function generateId(prefix = 'id', props?: Record<string, any>): string {
  if (props) {
    const propsHash = hashProps(props)
...
export function generateContentId(content: string, prefix = 'id'): string {
  const hash = content
    .split('')
...
export function generatePropsId(props: Record<string, any>, prefix = 'id'): string {
  const propsString = Object.entries(props)
    .filter(([_, value]) => value !== undefined && value !== null)
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([key, value]) => `${key}-${value}`)
