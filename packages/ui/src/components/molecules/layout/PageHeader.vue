<template>
  <div class="mb-4 sm:mb-8">
    ...
    <PhaseImageCard v-if="phaseData" :phase-data="phaseData" :dark-mode="currentDarkMode" />
    ...
    <div :class="[
      ...
      <div class="flex-1 min-w-0">
        <h1 :class="['font-bold text-gray-900 dark:text-white', titleSizeClass]">
          <span v-if="phaseData?.emoji" class="mr-2">{{ phaseData.emoji }}</span>
          <span v-else-if="phaseLoading" class="mr-2 animate-pulse">🌅</span>
          {{ title }}
        </h1>
        <p v-if="subtitle" :class="['text-gray-600 dark:text-gray-400 mt-1 sm:mt-2', subtitleSizeClass]">
          {{ subtitle }}
        </p>
      </div>
    ...
    <nav v-if="finalBreadcrumbs && finalBreadcrumbs.length > 0" class="mt-3 sm:mt-4" aria-label="Breadcrumb">
      <ol class="flex items-center space-x-2 text-xs sm:text-sm">
        <li v-for="(crumb, index) in finalBreadcrumbs" :key="index" class="flex items-center">
          <NuxtLink v-if="crumb.to && index < finalBreadcrumbs.length - 1" :to="crumb.to"
          ...
          <ChevronRight v-if="index < finalBreadcrumbs.length - 1" :size="12" class="text-gray-400 mx-1 sm:mx-2" />
        </li>
      </ol>
    </nav>
  </div>
