<script setup lang="ts">
  ...
  // Alineado con `useTheme` en `apps/web` (storage `theme`)
...
<template>
  <div class="relative">
    ...
    <div
      class="relative w-full h-32 sm:h-48 lg:h-64 mb-4 sm:mb-6 rounded-xl overflow-hidden shadow-lg group"
    >
      <img
        :src="phaseData.image"
        :alt="phaseData.label"
        class="object-cover w-full h-full transition-all duration-700 ease-out group-hover:scale-105"
        :class="[
          currentDarkMode
      ...
      <div
        class="absolute inset-0 bg-gradient-to-t transition-all duration-500"
        :class="
          currentDarkMode
      ...
      <div
        v-if="phaseData?.progress !== undefined"
        class="absolute bottom-0 left-0 right-0 h-1 bg-black/20"
      >
        <div
          class="h-full transition-all duration-500 ease-out"
          :class="currentDarkMode ? 'bg-primary-400' : 'bg-primary-500'"
    ...
    <div
      class="absolute top-4 sm:top-6 right-4 sm:right-6 flex items-center gap-3 sm:gap-4"
    >
      <!-- Icono de tiempo con tooltip moderno -->
      <div class="relative">
        <BaseTooltip
          v-if="phaseData?.timeUntilNext"
          variant="info"
          size="lg"
          :delay="300"
          :duration="200"
