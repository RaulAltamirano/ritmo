...
<template>
  <div class="space-y-4">
    ...
    <div
      v-for="n in count"
      :key="n"
      class="group relative bg-surface border border-outline rounded-xl p-6"
      :class="[variant === 'minimal' ? 'border-0 bg-transparent p-0' : '']"
    >
      <!-- Header Skeleton -->
      <div v-if="showHeader" class="flex items-center justify-between mb-4">
        <div class="flex items-center space-x-3">
          ...
          <BaseSkeleton
            ...
            :height="40"
            rounded="lg"
            aria-label="Loading header icon"
          />
          <div>
            ...
            <BaseSkeleton
              v-if="showHeaderSubtitle"
              variant="text"
              size="sm"
              :width="headerSubtitleWidth"
      ...
      <div v-if="showContent" class="space-y-4">
        ...
        >
          <div
            ...
            :key="item"
            class="bg-surface-raised/50 rounded-lg p-4"
          >
            <BaseSkeleton
              variant="circular"
              size="sm"
            ...
            <BaseSkeleton
              ...
              size="sm"
              :width="80"
              aria-label="Loading grid item value"
            />
          </div>
        </div>
      ...
      >
        ...
        <BaseSkeleton
          ...
          :height="32"
          rounded="md"
          aria-label="Loading footer action"
        />
      </div>
    </div>
  </div>
