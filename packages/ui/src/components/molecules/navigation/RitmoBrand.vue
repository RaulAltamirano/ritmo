<template>
  <div class="flex items-center space-x-3">
    <!-- Logo -->
    <RitmoLogo :size="logoSize" :variant="logoVariant" :aria-label="`${brandName} Logo`" />

    <!-- Texto de la marca -->
    <div v-if="showText" class="flex flex-col">
      <h1 :class="['font-bold font-display tracking-tight', titleClasses]">
        {{ brandName }}
      </h1>
      <p v-if="showTagline" :class="['text-sm font-medium', taglineClasses]">
        {{ tagline }}
      </p>
    </div>
  </div>
