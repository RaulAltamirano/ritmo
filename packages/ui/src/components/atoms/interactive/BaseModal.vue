...
import { computed, nextTick, onMounted, onUnmounted, ref, toValue, useSlots, watch } from 'vue'
...
<template>
  <Transition name="modal" appear @enter="handleEnter" @leave="handleLeave">
    ...
    >
      <!-- Backdrop -->
      <div
        class="absolute inset-0 transition-opacity" :class="[
          backdropClasses,
      ...
      <!-- Modal Container -->
      <div ref="modalRef" class="relative flex w-full flex-col" :class="[modalClasses]" @click.stop>
        <!-- Header -->
        <header v-if="$slots.header || title" class="flex shrink-0 items-center justify-between gap-3 p-4 sm:p-6" :class="[headerClasses]">
        ...
        <!-- Footer -->
        <footer v-if="$slots.footer" class="flex shrink-0 flex-wrap items-center justify-end gap-3 p-4 sm:p-6" :class="[footerClasses]">
          <slot name="footer" />
        </footer>
      </div>
    </div>
