<script setup lang="ts">
  ...
  const buttonClasses = computed(() => {
    const baseClasses = [
      'inline-flex items-center justify-center whitespace-nowrap font-medium rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2',
    ...
    const sizeClasses = {
      xs: props.iconOnly
        ? 'p-1 text-xs min-h-[1.5rem] min-w-[1.5rem]'
        : 'px-2 py-1 text-xs min-h-[1.5rem]',
      sm: props.iconOnly
        ? 'p-1.5 text-sm min-h-[2rem] min-w-[2rem]'
        : 'px-3 py-1.5 text-sm min-h-[2rem]',
      md: props.iconOnly
        ? 'p-2 text-sm min-h-[2.5rem] min-w-[2.5rem]'
        : 'px-4 py-2 text-sm min-h-[2.5rem]',
      lg: props.iconOnly
        ? 'p-3 text-base min-h-[3rem] min-w-[3rem]'
        : 'px-6 py-3 text-base min-h-[3rem]',
      xl: props.iconOnly
        ? 'p-4 text-lg min-h-[3.5rem] min-w-[3.5rem]'
...
<template>
  <button
    ...
    @pointerup="handleButtonPointerUp"
    @pointerleave="handleButtonPointerLeave"
  ...
  >
    ...
    >
      ...
      <BaseIcon
        ...
        :size="getIconSize()"
        color="current"
        :aria-label="`${iconAriaLabel} icono`"
        role="img"
