<script setup lang="ts">
  ...
  const errorClassNames = computed(() => {
    return [
  ...
  const requiredClassNames = computed(() => {
    return ['text-red-500 dark:text-red-400', 'font-medium', props.requiredClasses]
      .filter(Boolean)
      .join(' ')
  })
...
<template>
  <div class="w-full">
    ...
    <label
      ...
      :for="inputId"
      class="block mb-1"
      :class="[typography.getLabelTypography(props.size).value, labelClasses]"
    >
      {{ label }}
      <span
        v-if="required"
        class="ml-1"
        :class="[requiredClassNames]"
        aria-label="required"
        >*</span
      >
    </label>
    ...
    <div :class="containerClasses">
      ...
      <input
        ...
        class="w-full transition-all duration-200 focus:outline-none focus:ring-2"
        :class="[typography.getInputTypography(props.size).value, inputClasses]"
        @input="handleInput"
        @change="handleChange"
        @focus="handleFocus"
        @blur="handleBlur"
      ...
      <!-- Icono derecho -->
      <div
        v-if="rightIcon && !(clearable && inputValue && !disabled)"
      ...
      >
        <BaseIcon :icon="rightIcon" size="sm" color="muted" aria-hidden />
      </div>

      <!-- Botón de limpiar -->
      <BaseButton
        v-if="clearable && inputValue && !disabled && !rightIcon"
        size="sm"
        variant="ghost"
        :icon="X"
    ...
    <div
      ...
      :class="[typography.getFormTypography('error').value, errorClassNames]"
      role="alert"
      aria-live="polite"
    >
    ...
    <p
      ...
      :id="`${inputId}-hint`"
      class="mt-1"
      :class="[typography.getFormTypography('help').value, hintClassNames]"
    >
      {{ hint }}
    </p>
  </div>
