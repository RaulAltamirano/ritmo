...
<template>
  <div class="w-full">
    <!-- Label -->
    <label v-if="label" :for="inputId" class="block mb-1" :class="[
    ...
    <div :class="containerClasses">
      ...
      <input :id="inputId" ref="inputRef" v-model="inputValue" :type="type" :placeholder="placeholder"
        ...
        class="w-full transition-all duration-200 focus:outline-none focus:ring-2" :class="[
    ...
    <!-- Mensaje de error -->
    <div v-if="error" :id="`${inputId}-error`" class="mt-1 flex items-center" :class="[
    ...
    <!-- Texto de ayuda -->
    <p v-if="hint" :id="`${inputId}-hint`" class="mt-1" :class="[typography.getFormTypography('help').value, hintClasses]">
      {{ hint }}
    </p>
  </div>
</template>
