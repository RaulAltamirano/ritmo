...
<template>
  <div class="flex items-start">
    <div class="flex items-center h-5">
      <input :id="checkboxId" ref="checkboxRef" v-model="isChecked" :name="name" :value="value" :disabled="disabled"
        :required="required" :indeterminate="indeterminate" type="checkbox" class="sr-only focus:ring-2 focus:ring-offset-2" :class="[
          hasError ? 'focus:ring-red-500' : 'focus:ring-blue-500',
        ]" :aria-describedby="describedBy" :aria-invalid="hasError" :aria-required="required" @change="handleChange"
        @focus="handleFocus" @blur="handleBlur" />

      <label :for="checkboxId" class="relative flex h-5 w-5 cursor-pointer items-center justify-center rounded border transition-all duration-200" :class="[
        checkboxClasses,
        disabled ? 'cursor-not-allowed opacity-50' : 'hover:scale-105',
      ]" :aria-label="ariaLabel" @click="handleLabelClick">
        <!-- Checkmark icon -->
        <Check v-if="isChecked && !indeterminate" class="w-3 h-3 pointer-events-none text-white" :aria-hidden="true" />

        <!-- Indeterminate icon -->
        <Minus v-else-if="indeterminate" class="w-3 h-3 pointer-events-none text-white" :aria-hidden="true" />

        <!-- Focus ring -->
        <div class="absolute inset-0 rounded ring-2 ring-transparent transition-all duration-200 focus-within:ring-offset-2" :class="[
    ...
    <div v-if="$slots.default || label" class="ml-3 text-sm">
      <label v-if="label" :for="checkboxId" :class="[
        getLabelTypography(),
        labelClasses,
        disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer',
      ]">
        {{ label }}
        <span v-if="required" class="ml-1" :class="[requiredClasses]" aria-label="required">*</span>
      </label>

      <slot />

      <!-- Error message -->
      <div v-if="hasError" :id="`${checkboxId}-error`"
        class="mt-1 flex items-center" :class="[getErrorTypography(), errorClasses]" role="alert" aria-live="polite">
        <AlertCircle :class="[`w-4 h-3 mr-1 ${getColor('error', '600').value}`]" />
        {{ error }}
      </div>

      <!-- Help text -->
      <p v-if="hint" :id="`${checkboxId}-hint`" class="mt-1" :class="[getHintTypography(), hintClasses]">
        {{ hint }}
      </p>
    </div>
  </div>
