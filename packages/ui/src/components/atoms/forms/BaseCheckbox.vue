  ...
  <div class="flex items-start">
    <div class="flex items-center h-5">
      <input :id="checkboxId" ref="checkboxRef" v-model="isChecked" :name="name" :value="value" :disabled="disabled"
        :required="required" :indeterminate="indeterminate" type="checkbox" :class="[
          'sr-only',
      ...
      <label :for="checkboxId" :class="[
        'relative flex h-5 w-5 cursor-pointer items-center justify-center rounded border transition-all duration-200',
        checkboxClasses,
        ...
        <!-- Focus ring -->
        <div :class="[
          'absolute inset-0 rounded ring-2 ring-transparent transition-all duration-200',
    ...
    <div v-if="$slots.default || label" class="ml-3 text-sm">
      ...
      <div v-if="hasError" :id="`${checkboxId}-error`"
        :class="[getErrorTypography(), 'mt-1 flex items-center', errorClasses]" role="alert" aria-live="polite">
        <AlertCircle :class="[`w-4 h-3 mr-1 ${getColor('error', '600').value}`]" />
        {{ error }}
      </div>

      <!-- Help text -->
      <p v-if="hint" :id="`${checkboxId}-hint`" :class="[getHintTypography(), 'mt-1', hintClasses]">
        {{ hint }}
      </p>
    </div>
  </div>
...
const checkboxClasses = computed(() => {
  ...
  if (isChecked.value || props.indeterminate) {
    return `${getColor('primary', '600').value} ${getBorderColor('primary', '600').value} ${getTextColor('primary', '600').value} shadow-sm hover:${getColor('primary', '700').value} hover:${getBorderColor('primary', '700').value}`
  }
