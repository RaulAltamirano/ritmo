...
<template>
  <div :id="id" class="group relative overflow-hidden transition-all duration-300 ease-out" :class="[
  ...
  ]" :role="role" :tabindex="clickable ? 0 : undefined" :aria-label="ariaLabel" @click="handleClick"
    @keydown="handleKeyDown">
    <!-- Loading overlay -->
    <div v-if="loading" class="absolute inset-0 z-30 flex items-center justify-center" :class="[
      getSurface('overlay').value,
    ]">
      <div class="flex flex-col items-center gap-2">
        <BaseSpinner size="md" color="primary" />
        <span class="text-gray-900 dark:text-white font-medium" :class="[
          typography.getBodyTypography('base').value,
        ]">
          Cargando...
        </span>
      </div>
    ...
    <!-- Card header -->
    <div v-if="title || $slots.header" class="flex-shrink-0" :class="[
      variant === 'minimalist' ? '' : 'border-b',
      getBorderSemantic('secondary'),
    ]">
      <div :class="headerPaddingClasses">
        <slot name="header">
          <h3 class="text-gray-900 dark:text-white" :class="[
            typography.getCardTypography('title').value,
          ]">
            {{ title }}
          </h3>
          <p v-if="subtitle" class="text-gray-700 dark:text-gray-300 mt-1" :class="[
            typography.getCardTypography('subtitle').value,
          ]">
            {{ subtitle }}
          </p>
        </slot>
    ...
    <!-- Card footer -->
    <div v-if="$slots.footer" class="flex-shrink-0" :class="[
    ...
    <button v-if="dismissible" type="button" :aria-label="closeButtonAriaLabel" :class="closeButtonClasses"
      ...
      <X class="w-4 h-4 transition-colors duration-200 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"  />
    </button>

    <!-- Badge -->
    <div v-if="badge" class="absolute right-4 top-3 z-10">
      <BaseBadge :variant="badge.variant" :size="badge.size" :content="badge.text" :icon="badge.icon" />
    </div>

    <!-- Neumorphic inner shadow for depth -->
    <div v-if="variant === 'neumorphic'" class="absolute inset-0 pointer-events-none rounded-xl"
      :class="neumorphicInnerShadow"></div>
  </div>
</template>
