  ...
  <Transition name="alert" appear enter-active-class="transition-all duration-300 ease-out"
    ...
    <div v-if="show" :id="id" :class="[
      ...
      <div class="flex items-start gap-3 p-4">
        <!-- Status icon -->
        <div v-if="showIcon" :class="['flex-shrink-0 mt-0.5', iconClasses]">
        ...
        <div class="flex-1 min-w-0">
          <!-- Title -->
          <h3 v-if="title" :id="titleId" :class="['font-semibold leading-5', titleClasses]">
            {{ title }}
          </h3>

          <!-- Message -->
          <div v-if="message" :id="messageId" :class="['text-sm leading-5 mt-1', messageClasses]">
            {{ message }}
          </div>
          ...
          <div v-if="actions && actions.length > 0" class="mt-3 flex flex-wrap gap-3">
            <button v-for="(action, index) in actions" :key="index" type="button" :class="[
...
import { useColors, useId } from '@ritmo/ui'
