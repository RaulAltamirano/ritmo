<script setup lang="ts">
  ...
  const contentClasses = computed(() => {
    return ['flex-shrink-0', 'leading-none', 'font-medium'].join(' ')
  })

  const iconClasses = computed(() => {
    return ['flex-shrink-0', 'transition-transform', 'duration-200'].join(' ')
  })
...
<template>
  ...
  >
    ...
    <span
      v-if="hasContent"
      :class="[contentClasses, { 'mr-1': rightIconComponent || showCount }]"
    >
      <slot>{{ content }}</slot>
    </span>
