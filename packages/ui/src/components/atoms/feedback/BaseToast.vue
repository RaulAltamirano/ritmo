<template>
  ...
  <div :class="toastContainerClasses" class="fixed z-[9999] pointer-events-none">
    <!-- Toast individual -->
    <Transition v-for="toast in toasts" :key="toast.id" :name="transitionName" appear @enter="handleEnter"
      @leave="handleLeave">
      <div v-if="toast.show" :id="toast.id" :class="[
        'pointer-events-auto mb-3 transition-all duration-300 ease-out max-w-sm relative',
        getToastClasses(toast.variant),
        getToastSizeClasses(toast.size || 'md'),
      ]" role="alert" :aria-live="getAriaLive('medium')" :aria-atomic="true"
        :aria-label="toast.closeButtonAriaLabel || 'Notificación'" @mouseenter="pauseAutoHide(toast.id)"
        @mouseleave="resumeAutoHide(toast.id)" @focus="pauseAutoHide(toast.id)" @blur="resumeAutoHide(toast.id)">
        <!-- Contenido del toast -->
        <div class="flex items-start gap-3">
          <!-- Icono del toast -->
          <div v-if="toast.showIcon" class="flex-shrink-0">
            <component :is="getVariantIcon(toast.variant)" :class="['w-5 h-5', getIconClasses(toast.variant)]"
              aria-hidden="true" />
          </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div v-if="toast.actions && toast.actions.length > 0" class="flex gap-2 mt-3">
              <BaseButton v-for="action in toast.actions" :key="action.label" :variant="(action.variant === 'info' ? 'primary' : action.variant) ||
                'secondary'
                " size="sm" :aria-label="action.label" @click="action.onClick">
                {{ action.label }}
              </BaseButton>
            </div>
          </div>
          ...
          <button v-if="toast.dismissible" type="button"
            :aria-label="toast.closeButtonAriaLabel || 'Cerrar notificación'" :class="closeButtonClasses"
        ...
        <div v-if="toast.showProgress"
          class="absolute bottom-0 left-0 right-0 h-1 bg-gray-200 dark:bg-gray-700 rounded-b-lg overflow-hidden"
          role="progressbar" :aria-valuenow="getReactiveProgress(toast.id).value" aria-valuemin="0" aria-valuemax="100"
          :aria-label="`Progreso: ${getReactiveProgress(toast.id).value}%`">
          <!-- Barra de progreso fluida -->
          <div class="h-full transition-all duration-300 ease-out rounded-r-full"
            :class="getProgressBarClasses(toast.variant)" :style="{
...
const getToastClasses = (variant: string) => {
  ...
  const baseClasses =
    'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg hover:shadow-xl rounded-xl p-4 transition-all duration-300'
...
const closeButtonClasses = computed(() => {
  ...
  switch (props.closeButtonStyle) {
    ...
    case 'subtle':
      return `${baseClasses} hover:bg-gray-100 dark:hover:bg-gray-700 hover:scale-105 focus:ring-2 focus:ring-gray-400/30 focus:ring-offset-1`
