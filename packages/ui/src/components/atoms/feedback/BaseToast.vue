...
<template>
  ...
  <div :class="toastContainerClasses" class="fixed z-[9999] pointer-events-none">
    ...
    <Transition v-for="toast in toasts" :key="toast.id" :name="transitionName" appear @enter="handleEnter"
      @leave="handleLeave">
      <div v-if="toast.show" :id="toast.id" class="pointer-events-auto mb-3 transition-all duration-300 ease-out max-w-sm relative" :class="[
        ...
        <div class="flex items-start gap-3">
          ...
          <div v-if="toast.showIcon" class="flex-shrink-0">
            <component :is="getVariantIcon(toast.variant)" class="w-5 h-5" :class="[getIconClasses(toast.variant)]"
              aria-hidden="true" />
          </div>

          <!-- Contenido principal -->
          <div class="flex-1 min-w-0">
            <div v-if="toast.title" class="mb-1" :class="[
              typography.getToastTypography('title').value,
              getTextSemantic('primary').value,
            ]">
              {{ toast.title }}
            </div>
            <div v-if="toast.message" class="opacity-90" :class="[
